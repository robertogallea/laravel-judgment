...
- **[Judgments](#declaring-a-judgment)** constructed with their Subject, declaring explicit Evidence, with end-user text marked [untrusted](#untrusted-evidence).
- **[Typed Questions](#questions)**: Likelihood, Classification, Rating and Likelihood Set, [read](#reading-an-assessment) with threshold helpers and a package-defined Confidence.
- **[Pure Decisions](#deciding)** mapping an Assessment and Subject facts to a backed-enum Outcome, generated by [`make:judgment` and `make:decision`](#generating-judgments-and-decisions).
- **[Failures as data](#when-the-engine-fails)**: an Engine failure is thrown or returned as `Unassessed`, never mapped to a default Outcome.
- **[Engines](#engines)**: TypeSafe's hosted Jev and a self-hosted [Laya](#laya) server, with pinned models and several connections.
- **[Caching](#caching)** of identical Evidence and Questions, opt-in per Judgment.
- **[Persisted Assessments](#persisted-assessments)** with their Evidence, answers, Provenance and Outcome, for audit and replay.
- **[Queued assessment](#queued-assessment)** off the request cycle.
- **[Review and Resolution](#review-and-resolution)** of uncertain Outcomes by a person, recorded next to the automatic Outcome.
- **[Calibration](#calibration)** of a Decision's thresholds against labelled cases with `judgment:eval`.
- **[Coding-agent skills](#coding-agents-laravel-boost)** for Laravel Boost that write Judgments and Decisions from a plain-language description.
- **[Events and logging](#events-and-logging)** for every assessment and Review.
- **[Testing fakes](#testing)** that script answers without an Engine: `Assessment::fake()` and `Judge::fake()`.
...
The package ships [Laravel Boost](https://github.com/laravel/boost) resources, so a coding agent can write Judgments and Decisions from a plain-language description:
